...
exports.cleanQueue = exports.getQueueStatus = exports.resumeQueue = exports.pauseQueue = exports.resumeScan = exports.stopScan = exports.scanQueue = void 0;
...
const redisClient = new ioredis_1.default({
    host: config_1.config.redis.url ? new URL(config_1.config.redis.url).hostname : 'localhost',
    port: config_1.config.redis.url ? parseInt(new URL(config_1.config.redis.url).port || '6379', 10) : 6379,
...
// Tarama kuyruğu
exports.scanQueue = new bull_1.default('scan-queue', {
    redis: {
...
const axiosOptions = {
    timeout: config_1.config.scanner.timeout,
    headers: {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            for (const type of jobTypes) {
                let jobs;
                try {
                    jobs = type === 'active' ? yield exports.scanQueue.getActive() :
                        type === 'waiting' ? yield exports.scanQueue.getWaiting() :
                            yield exports.scanQueue.getDelayed();
                }
                catch (err) {
                ...
                for (const job of jobs) {
                    try {
                        ...
                        if (data && data.scanId === scanId) {
                            if (type === 'active') {
                                // Aktif işi durdur
                                yield job.moveToFailed(new Error('Tarama kullanıcı tarafından durduruldu'), true);
                            }
                            else {
            ...
            try {
                yield redisClient.set(`stop:${scanId}`, Date.now().toString(), 'EX', 86400); // 24 saat boyunca tut
            }
            catch (err) {
...
exports.scanQueue.process((job) => __awaiter(void 0, void 0, void 0, function* () {
    // İş data'sını al
    const data = job.data;
    ...
    try {
        const isStopped = yield redisClient.exists(`stop:${data.scanId}`);
        if (isStopped) {
    ...
    try {
        ...
        yield (0, scanService_1.updateScanStatus)(scanConfig.scanId, 'running');
        ...
        for (let i = scanConfig.startIndex; i < scanConfig.domains.length; i += scanConfig.batchSize) {
            ...
            if (i % (10 * scanConfig.batchSize) === 0 || currentTime - lastStatusCheckTime > 120000) {
                ...
                try {
                    const scan = yield (0, scanService_1.getScan)(scanConfig.scanId);
                    if (!scan || scan.status !== 'running') {
                        console.log(`Tarama durumu artık 'running' değil (${scan === null || scan === void 0 ? void 0 : scan.status}), işlem durduruluyor: ScanID=${scanConfig.scanId}`);
                        stoppedScans.add(scanConfig.scanId);
                        break;
                    }
                    // Redis'te durdurma kaydı kontrolü
                    const isStopped = yield redisClient.exists(`stop:${scanConfig.scanId}`);
                    if (isStopped) {
            ...
            if (stoppedScans.has(scanConfig.scanId)) {
                ...
                yield (0, scanService_1.updateScannedDomains)(scanConfig.scanId, i);
                yield (0, scanService_1.updateScanStatus)(scanConfig.scanId, 'paused');
                return {
            ...
            const batchResults = yield processDomainsInBatch(batchDomains, scanConfig, i);
            ...
            yield (0, scanService_1.updateScannedDomains)(scanConfig.scanId, i + batchDomains.length - 1);
            ...
            yield updateJobStatus(job, scanConfig.scanId, i);
        }
        // Tarama tamamlandı
        console.log(`Tarama tamamlandı: ScanID=${scanConfig.scanId}, tarandı=${scannedCount}, bulundu=${foundCount}`);
        yield (0, scanService_1.updateScanStatus)(scanConfig.scanId, 'completed');
        return {
            totalScanned: scannedCount,
            totalFound: foundCount,
            results: results,
        ...
        try {
            yield (0, scanService_1.updateScanStatus)(scanConfig.scanId, 'failed');
            stoppedScans.add(scanConfig.scanId);
        }
...
function processDomainsInBatch(batchDomains, data, startIndex) {
    return __awaiter(this, void 0, void 0, function* () {
        const results = [];
        let scannedCount = 0;
        let foundCount = 0;
        ...
        for (let j = 0; j < batchDomains.length; j++) {
            ...
            try {
                ...
                // URL'leri işle
                const domainResults = yield processDomainUrls(urls, domain, data);
                // Sonuçları ekle
...
function processDomainUrls(urls, domain, config) {
    return __awaiter(this, void 0, void 0, function* () {
        const results = [];
        ...
        for (let i = 0; i < httpsUrls.length; i += urlBatchSize) {
            ...
            try {
                ...
                const batchResults = yield Promise.all(urlPromises);
                ...
                for (const result of batchResults) {
                    if (result) {
                        ...
                        try {
                            ...
                            yield (0, scanService_1.addScanResult)(config.scanId, result);
                        }
                        catch (err) {
            ...
            if (i > 0 && i % 20 === 0) {
                try {
                    ...
                    const scan = yield (0, scanService_1.getScan)(config.scanId);
                    if (!scan || scan.status !== 'running') {
...
// Tek bir URL'yi işle
function processUrl(url, domain, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        while (retryCount <= maxRetries) {
            try {
                ...
                // HTTP isteği yap
                const response = yield axios_1.default.get(url, axiosOptions);
                ...
                const errorMsg = (error === null || error === void 0 ? void 0 : error.message) || 'Bilinmeyen hata';
                ...
                if (retryCount <= maxRetries) {
                    ...
                    try {
                        ...
                        console.error(`HTTP denemesi de başarısız oldu: ${httpUrl} - ${(httpError === null || httpError === void 0 ? void 0 : httpError.message) || 'Bilinmeyen hata'}`);
                    }
                    return null;
                }
                else {
...
function resumeScan(scanId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield redisClient.del(`stop:${scanId}`);
            // Tarama bilgilerini al
            const scan = yield (0, scanService_1.getScan)(scanId);
            if (!scan) {
            ...
            yield cleanupExistingJobs(scanId);
            // Domain listesini al
            ...
            if (startIndex >= domainList.domains.length) {
                ...
                yield (0, scanService_1.updateScanStatus)(scanId, 'completed');
                return true;
            }
            console.log(`Tarama kaldığı yerden devam edecek, başlangıç indeksi: ${startIndex}/${domainList.domains.length}`);
            // Varsayılan değerler
            const batchSize = scan.config.batchSize || config_1.config.scanner.batchSize;
            const urlBatchSize = scan.config.urlBatchSize || config_1.config.scanner.urlBatchSize;
            const retries = scan.config.retries || config_1.config.scanner.retries;
            const timeout = scan.config.timeout || config_1.config.scanner.timeout;
            // Yeni job oluştur ve kuyruğa ekle
            const job = yield exports.scanQueue.add({
                scanId: scan.id,
                ...
                concurrency: scan.config.concurrency || config_1.config.scanner.concurrency,
                timeout: timeout,
                batchSize: batchSize,
            ...
            // Tarama durumunu running olarak güncelle
            yield (0, scanService_1.updateScanStatus)(scan.id, 'running');
            return true;
        }
        catch (error) {
...
function cleanupExistingJobs(scanId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Tüm job türlerini al
            const activeJobs = yield exports.scanQueue.getActive();
            const waitingJobs = yield exports.scanQueue.getWaiting();
            const delayedJobs = yield exports.scanQueue.getDelayed();
            ...
            for (const job of activeJobs) {
                ...
                if (data && data.scanId === scanId) {
                    try {
                        yield job.moveToFailed(new Error('Tarama yeniden başlatıldı'), true);
                        yield job.remove();
...
function pauseQueue() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield exports.scanQueue.pause(true); // true - global olarak durdur
...
function resumeQueue() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield exports.scanQueue.resume(true); // true - global olarak başlat
...
function getQueueStatus() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const counts = yield exports.scanQueue.getJobCounts();
            const isPaused = yield exports.scanQueue.isPaused();
            return {
                counts,
                isPaused,
            };
...
function cleanQueue() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            // Önce kuyruğu durdur
            yield exports.scanQueue.pause();
            // Tüm işleri al
            const allJobs = yield exports.scanQueue.getJobs(['active', 'waiting', 'delayed', 'failed', 'completed']);
            console.log(`Toplam ${allJobs.length} iş temizlenecek`);
            // Tüm işleri sil
            for (const job of allJobs) {
                try {
                    if (yield job.isActive()) {
                        yield job.moveToFailed(new Error('Kuyruk temizleniyor'), true);
                    }
                    yield job.remove();
                }
                catch (error) {
            ...
            yield exports.scanQueue.resume();
        ...
        try {
            ...
            if (currentIndex % (10 * (job.data.batchSize || 10)) === 0) {
                try {
                    ...
                    const checkInKeys = yield redisClient.keys(`job-status:${scanId}:*`);
